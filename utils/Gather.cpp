...
void cgather_batch(float* preds, long* candidate_indices, float* scores, int R, int B, int N, int batch_size, int n_threads)
{   
    ...
    for(int idx = 0; idx < batch_size; ++idx)
    {
        ...
        for(int jdx = 0; jdx < R; ++jdx)
        {
            ...
            {
                scores[offset + kdx] += preds[raw_offset + candidate_indices[idx_offset + kdx]];
            }
        }
    }
}
